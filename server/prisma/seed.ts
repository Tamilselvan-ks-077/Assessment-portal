import { PrismaClient, UserRole, QuestionType, AssessmentStatus } from '@prisma/client';
...
async function main() {
  ...
  const assessment = await prisma.assessment.create({
    data: {
      ...
      description:
        'Comprehensive evaluation test covering TypeScript, React, REST API design, PostgreSQL queries, Docker, and Cloud Native Security practices.',
      category: 'Software Engineering',
      instructions:
        'Read every question carefully. For multiple choice questions, all correct options must be selected. Time limit is 30 minutes.',
      accessCode: 'DEV2026',
  ...
  const q3 = await prisma.question.create({
    data: {
      ...
      order: 3,
      explanation:
        'False. For very small tables or queries fetching a large fraction of the table, sequential scans are often faster because they avoid random I/O.',
      isRequired: true,
      options: {
        create: [
  ...
  const q5 = await prisma.question.create({
    data: {
      ...
      order: 5,
      explanation:
        'Optimistic concurrency checks for conflicts before committing (e.g. via version field), while pessimistic locking locks the row/table immediately to prevent concurrent modifications.',
      isRequired: true,
      options: {
        create: [],
      },
      ...
      answers: {
        create: [
          ...
          },
          {
            questionId: q5.id,
            textAnswer:
              'Optimistic concurrency checks version numbers at commit time without blocking reads/writes, while pessimistic locking holds exclusive row locks during the transaction lifetime.',
            isCorrect: true,
            marksAwarded: 5,
            answeredAt: new Date(),
          },
